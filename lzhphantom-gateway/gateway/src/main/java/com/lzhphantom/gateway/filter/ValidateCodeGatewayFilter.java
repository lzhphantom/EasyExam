...
import com.lzhphantom.core.exception.ValidateCodeException;
...
public class ValidateCodeGatewayFilter extends AbstractGatewayFilterFactory<Object> {
    ...
    public GatewayFilter apply(Object config) {
        return (exchange, chain) -> {
            ...
            boolean isIgnoreClient = configProperties.getIgnoreClients().contains(LzhphantomWebUtils.getClientId(request));
            try {
                if (!isIgnoreClient) {
                    checkCode(request);
                }
                ...
                return response.writeWith(Mono.create(monoSink -> {
                    try {
                        DataBuffer dataBuffer = response.bufferFactory().wrap(objectMapper.writeValueAsBytes(errMsg));
