package com.lzhphantom.job;

import com.lzhphantom.job.properties.XxlExecutorProperties;
import com.lzhphantom.job.properties.XxlJobProperties;
...
public class JobAutoConfiguration {
     ...
     */
    private static final String LZHPHANTOM_JOB_ADMIN = "lzhphantom-job-admin";
    ...
    public XxlJobSpringExecutor xxlJobSpringExecutor(XxlJobProperties xxlJobProperties, Environment environment,
        ...
        // 如果配置为空则获取注册中心的服务列表 "http://lzhphantom-xxl:9080/xxl-job-admin"
        if (!StringUtils.hasText(xxlJobProperties.getAdmin().getAddresses())) {
            String serverList = discoveryClient.getServices().stream().filter(s -> s.contains(LZHPHANTOM_JOB_ADMIN))
                    .flatMap(s -> discoveryClient.getInstances(s).stream()).map(instance -> String
                            .format("http://%s:%s/%s", instance.getHost(), instance.getPort(), LZHPHANTOM_JOB_ADMIN))
